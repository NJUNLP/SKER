...
@DatasetReader.register("definition_reader")
class DefinitionReader(DatasetReader):
    def __init__(self,
                 idiom_vector_path: str,
                 idiom_definition_path: str,
        ...
        with open(idiom_definition_path) as fh:
            idiom_definition = json.load(fh)
        self.idiom_definition = idiom_definition

    @staticmethod
    def _to_tokens(items: List[str]):
        return [Token(item) for item in items]
    ...
    def text_to_instance(self,
        ...
        for index, current_candidates in enumerate(candidates):
            before_part_tokens = self._to_tokens(split_tokens[0])
            for before_part in split_tokens[1:index+1]:
                before_part_tokens += [Token('[UNK]')] + self._to_tokens(before_part)
            after_part_tokens = self._to_tokens(split_tokens[index+1])
            for after_part in split_tokens[index+2:]:
                after_part_tokens += [Token('[UNK]')] + self._to_tokens(after_part)
            ...
            candidate_text_tokens = [
                TextField(self._to_tokens(self.tokenizer.tokenize(option)),
                          self.content_token_indexer)
                for option in current_candidates
            ]
            candidate_text_tokens = ListField(candidate_text_tokens)

            # 候选成语（定义）
            candidate_definition_tokens = [
                TextField(
                    self._to_tokens(
                        self.tokenizer.tokenize(self.idiom_definition[option])
                    ),
                    self.content_token_indexer
                )
                for option in current_candidates
            ]
            candidate_definition_tokens = ListField(candidate_definition_tokens)

            fields = {
                "content": content_field,
                "blank_indices": blank_index_field,
                "candidates": candidate_field,
                "candidate_texts": candidate_text_tokens,
                "candidate_definitions": candidate_definition_tokens
            }
            ...
            meta = {
                ...
                "candidate_definitions": [self.idiom_definition[option] for option in current_candidates]
            }
            ...
            if ground_truths:
                ...
                meta["answer"] = ground_truths[index]
